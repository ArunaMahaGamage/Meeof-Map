...
import com.meeof.meeof.activity.SearchTagsActivity;
 ...
 */
public class UpdatesRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private static final String TAG = UpdatesRecyclerAdapter.class.getSimpleName();
    private Context mContext;
    public Data updateData;
    public List<Array_updates> updatesList;
...
//    private final ViewBinderHelper viewBinderHelper = new ViewBinderHelper();
    ...
    public UpdatesRecyclerAdapter(Context context, Data data) {
        updateData = data;
        this.updatesList = updateData.getArray_updates();
    ...
    public void updateLikeCount(Context context, int id, int count, boolean isLike) {
        Log.d(TAG, "UpdatesAdapter setting " + updatesList.size());
        for (Array_updates updateItem : updatesList) {
            if (updateItem.getUpdateid() == id) {
                updateItem.setCountLikes(count);
                if (isLike) {
                    updateData.getWhatDoILike().add(id);
                } else {
                    updateData.getWhatDoILike().remove(id);
                }
                break;
            }
        }
...
//                .inflate(R.layout.item_friend_deletable, parent, false);
        ...
        return new UpdatesRecyclerAdapter.MyViewHolderUpdates(itemView);

    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        final Array_updates updateItem = updatesList.get(position);
        final UpdatesRecyclerAdapter.MyViewHolderUpdates myViewHolder = (UpdatesRecyclerAdapter.MyViewHolderUpdates) holder;
...
//        }
        ...
        for (int i = 0; i < updateData.getWhatDoILike().size(); i++) {
            if (updateData.getWhatDoILike().contains(updateItem.getUpdateid())) {
                myViewHolder.likedBtn.setVisibility(View.VISIBLE);
                myViewHolder.unLiked.setVisibility(View.GONE);
                break;
            } else {
        ...
        myViewHolder.popUpMenuLl.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (updateItem.getUser_id() == updateData.getMyID()) {
        ...
        myViewHolder.commentsBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                args.putInt(Constant.USER_ID, updateData.getMyID());
                args.putBoolean(Constant.IS_LIKE, updateData.getWhatDoILike().contains(updateItem.getUpdateid()));
                intent.putExtra("BUNDLE_UPDATE", args);

                Log.d(TAG, "IS LIKE: " + updateData.getWhatDoILike().toString());
                Log.d(TAG, "IS LIKE: " + updateData.getWhatDoILike().contains(updateItem.getUpdateid()));
    ...
    public void addItem(Array_updates updateItemData) {
...
//        return listItem.size();
    ...
    private void showPopUpMenu(final boolean isHost, View anchor, final Array_updates updateItem) {
