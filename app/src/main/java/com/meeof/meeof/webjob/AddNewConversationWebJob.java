...
public class AddNewConversationWebJob extends Job {
    ...
    public AddNewConversationWebJob(String token, ArrayList<Integer> friendsIds, String Message) {
...
//            .build();
            ...
            formBody.add("message", Message);
            if (friendsIds != null && friendsIds.size() > 0) {
                for (int i = 0; i < friendsIds.size(); i++) {
                    formBody.add("friends" + "[" + i + "]", String.valueOf(friendsIds.get(i)));
                }
            }
