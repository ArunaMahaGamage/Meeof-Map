...
public class GetImportantNotificationsJob extends Job {
    ...
    public GetImportantNotificationsJob(String token) {
...
//                    sharedEditor.putLong(Constant.LAST_SYNC_TIME,(System.currentTimeMillis()));
                    ...
                    if(notifications.length() > 0){
                        EventBus.getDefault().post(new GetNotificationsWebJobCompletedOtherNotifications(Constant.SUCCESS, notifications));
                    }else{
                        EventBus.getDefault().post(new GetNotificationsWebJobCompletedOtherNotifications(Constant.SUCCESS, null));

                    }

                } catch (Exception e) {
                    Log.i(TAG, "error at converting to JsonArray " + e.getMessage().toString());
                    EventBus.getDefault().post(new GetNotificationsWebJobCompletedOtherNotifications(Constant.ERROR, null));
                }
            }
        } catch (Exception e) {
